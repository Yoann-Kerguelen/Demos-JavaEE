	...
	<div class="flex-container">
		...
		<main>
			<div class="page-title">
				<h1 id="title">Création d'une sortie</h1>
			</div>
			<div class="table">
				<form action="<%=request.getContextPath()%>/ajouter-sortie"
					method="post">
					<div>
						<label for="trip-name" id="trip-name">Nom de la sortie</label> <input
							type="text" name="trip-name" required="required">
					</div>
					<div>
					...
					<div>
						<label for="date">Date</label> <input type="date" id="date"
							...
							data-date-format="DD MMMM YYYY" lang="fr-FR" required="required">
					</div>
					<div>
						<label for="tripHour" id="tripHour">Heure de la sortie</label> <input
							type="time" name="tripHour" required="required">
					</div>
					...
					<div class="multiselect">
						...
						<div id="checkboxes">
							...
							<label for="<%=participant.getId()%>"> <input
								...
								type="checkbox" id="<%=participant.getId()%>" /><%=participant.getUsername()%></label>
							<%
							}
							%>
						</div>
					...
					<div>
						<button type="submit" id="add-trip-btn" name="add-trip-btn">Ajouter
							la sortie</button>
					</div>
				</form>
			</div>
